...
function draw() {
	...
	if (blocks.size() < 0) {
		for (var b=blocks.size();b>=0;b--) {
			var blc = blocks.get(b);
			if (player.sprite.position.x < blc.position.x + blc.width &&
				player.sprite.position.x + player.sprite.width > blc.position.x &&
				player.sprite.position.y < blc.position.y + blc.height &&
				player.sprite.height + player.sprite.position.y > blc.position.y) {
